...
for (m in 1:12) {
  ...
  preds = mutate(z, pred = ifelse(between(sst, clim$min_sst, clim$max_sst) & between(sss, clim$min_sss, clim$max_sss), 1, 0))
  
  p = ggplot() + 
    geom_stars(data=preds, aes(fill=pred)) +
    theme_bw() +
    theme(legend.position="none", axis.title = element_blank()) +
    ggtitle(paste(species, "mean +- 1sd: month", m, sep=" "))
...
## names(z) = c("sst", "sss")
## 
## preds = mutate(z, pred = ifelse(between(sst, 8.07, 28.5) & between(sss, 7.48, 37.2), 1, 0))
## 
## plot(preds["pred"])
## 
## ggplot() + 
